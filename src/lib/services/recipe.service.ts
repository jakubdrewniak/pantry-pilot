...
/**
 * Custom error class for user without household
 * Thrown when user attempts recipe operations without belonging to a household
 */
export class NoHouseholdError extends Error {
  constructor(message = 'User must belong to a household to perform this action') {
    super(message)
...
export class RecipeService {
   ...
   * @throws NoHouseholdError if user doesn't belong to any household
   *
   * Note: Every user should have a household (created automatically on registration).
   * If this throws, it indicates a data integrity issue or the user was created before
   * the household auto-creation trigger was implemented.
   */
  async getUserHouseholdId(userId: string): Promise<string> {
  ...
  async createRecipe(userId: string, input: CreateRecipeInput): Promise<Recipe> {
    // Get user's household_id (throws NoHouseholdError if not found)
  ...
  async bulkDeleteRecipes(userId: string, recipeIds: string[]): Promise<BulkDeleteRecipesResponse> {
    // 1. Get user's household_id (throws NoHouseholdError if not found)
    const householdId = await this.getUserHouseholdId(userId)
  ...
  async listRecipes(
    ...
    // 1. Get household_id (throws NoHouseholdError if not found)
    const householdId = await this.getUserHouseholdId(userId)
